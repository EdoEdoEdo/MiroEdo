"""
Zep tools service (ported from MiroFish `app/services/zep_tools.py`).

Wraps Zep graph search, node/edge fetching, and high-level "insight"/
"panorama"/"quick" search tools used by the Report Agent.

Requires `zep_cloud` to be installed. Instantiation also requires a configured
`EngineConfig` (passed via `set_engine_config()` or via the agent factory).
"""
...
from app.engine.utils.locale import get_locale, t
from app.engine.zep.paging import fetch_all_nodes, fetch_all_edges

logger = get_logger("miroedo.engine.zep_tools")
...
class ZepToolsService:
    ...
    def __init__(
        ...
        self.config = config
        self.api_key = api_key or (config.zep_api_key if config else "")
        if not self.api_key:
            raise ValueError("ZEP_API_KEY not configured")

        self.client = Zep(api_key=self.api_key)
        self._llm_client = llm_client
        self._simulations_dir = simulations_dir or (config.simulations_dir if config else "")
        logger.info("ZepToolsService initialized")
    
    @property
    def llm(self) -> LLMClient:
        """Lazy init for the LLM client used by InsightForge sub-query generation."""
        if self._llm_client is None:
            if self.config is None:
                raise RuntimeError(
                    "LLMClient not provided and EngineConfig not set on ZepToolsService"
                )
...
返回JSON格式的子问题列表。"""
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        """Load OASIS agent profiles for a given simulation."""
        import os
        import csv

        # Use configured simulations_dir; fall back to relative path for back-compat
        if self._simulations_dir:
            sim_dir = os.path.join(self._simulations_dir, simulation_id)
        else:
