"""
Report Agent (ported from MiroFish `app/services/report_agent.py`).

Implements a ReACT-mode report generator on top of Zep + LLM tools.
Requires Zep to be enabled (Report Agent is part of the Zep-enabled
deep-report feature in MiroEdo).

Usage:
    from app.engine.report import ReportAgent
    agent = ReportAgent(
...
from app.engine.utils.llm_client import LLMClient
from app.engine.utils.logger import get_logger
from app.engine.utils.locale import get_language_instruction, t
from app.engine.zep.tools import (
    ZepToolsService,
    SearchResult,
    InsightForgeResult,
    PanoramaResult,
...
logger = get_logger("miroedo.engine.report_agent")
...
_DEFAULT_REPORTS_DIR = os.environ.get(
    "REPORTS_DIR",
...
def configure(config: EngineConfig) -> None:
    """Inject EngineConfig overrides for module-level defaults.

    Affects `_DEFAULT_REPORTS_DIR`, `ReportAgent.MAX_TOOL_CALLS_PER_SECTION`,
    and `ReportAgent.MAX_REFLECTION_ROUNDS` / `ReportManager.REPORTS_DIR`.
    """
    global _DEFAULT_REPORTS_DIR
    if config.reports_dir:
        _DEFAULT_REPORTS_DIR = os.path.abspath(config.reports_dir)
    # ReportAgent/ReportManager pick up the new dir on next access
    ReportAgent.MAX_TOOL_CALLS_PER_SECTION = config.report_max_tool_calls
    ReportAgent.MAX_REFLECTION_ROUNDS = config.report_max_reflection_rounds
    ReportManager.REPORTS_DIR = _DEFAULT_REPORTS_DIR
...
class ReportLogger:
    ...
    def __init__(self, report_id: str):
        ...
        self.log_file_path = os.path.join(
            _DEFAULT_REPORTS_DIR, report_id, 'agent_log.jsonl'
        )
...
class ReportConsoleLogger:
    ...
    def __init__(self, report_id: str):
        ...
        self.report_id = report_id
        self.log_file_path = os.path.join(
            _DEFAULT_REPORTS_DIR, report_id, 'console_log.txt'
        )
        self._ensure_log_file()
...
class ReportAgent:
    ...
    # 最大工具调用次数（每个章节） - overridable via REPORT_AGENT_MAX_TOOL_CALLS
    MAX_TOOL_CALLS_PER_SECTION = int(os.environ.get("REPORT_AGENT_MAX_TOOL_CALLS", "3"))
    # 单个报告最大反思轮数
    MAX_REFLECTION_ROUNDS = int(os.environ.get("REPORT_AGENT_MAX_REFLECTION_ROUNDS", "2"))

    # 对话中的最大工具调用次数 - overridable via REPORT_AGENT_MAX_TOOL_CALLS_PER_CHAT
    MAX_TOOL_CALLS_PER_CHAT = int(os.environ.get('REPORT_AGENT_MAX_TOOL_CALLS_PER_CHAT', '2'))

    # PanoramaSearch limit - overridable via REPORT_AGENT_PANORAMA_LIMIT
    PANORAMA_LIMIT = int(os.environ.get('REPORT_AGENT_PANORAMA_LIMIT', '50'))
    ...
    def __init__(
        ...
        Args:
            ...
            simulation_requirement: human-readable simulation goal
            config: EngineConfig (optional; required if zep_tools not provided)
            llm_client: LLM client (optional)
            zep_tools: pre-built ZepToolsService (optional)
        """
        ...
        self.zep_tools = zep_tools or ZepToolsService(config=config)
