...
Adapted from MiroFish `app/services/simulation_runner.py`.

Changes:
- Replaces global `Config` with `EngineConfig` injected via
  `SimulationRunner.configure(engine_config)`
- `RUN_STATE_DIR` and `SCRIPTS_DIR` derived from `EngineConfig.workspace_dir`
  and `app.engine.simulation` package directory respectively
- `ZepGraphMemoryManager` is fully optional; only loaded when
  `engine_config.zep_enabled and enable_graph_memory_update=True`
- Subprocess invokes `python -m app.engine.simulation.parallel_runner`
"""
...
from app.engine.utils.locale import get_locale, set_default_locale as set_locale
from app.engine.ipc.simulation_ipc import SimulationIPCClient, CommandType, IPCResponse

# ZepGraphMemoryManager is optional; lazily imported when needed.
ZepGraphMemoryManager = None  # type: ignore


def _load_zep_graph_memory_manager():
    """Lazy import for the optional Zep graph memory updater."""
    global ZepGraphMemoryManager
    if ZepGraphMemoryManager is not None:
        return ZepGraphMemoryManager
    try:
        from app.engine.zep.graph_memory_updater import (  # type: ignore
            ZepGraphMemoryManager as _Mgr,
        )

        ZepGraphMemoryManager = _Mgr
        return _Mgr
    except Exception as exc:
        logger.warning(
            f"ZepGraphMemoryManager unavailable ({exc}); graph memory updates disabled"
        )
        return None


logger = get_logger("miroedo.engine.simulation_runner")
...
class SimulationRunner:
    ...
    # Runtime configuration (set via SimulationRunner.configure())
    _engine_config: Optional[EngineConfig] = None

    # Where simulations are stored (per-simulation subdir).
    # Defaults to <workspace>/data/simulations; can be overridden via configure().
    RUN_STATE_DIR: str = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../../data/simulations")
    )

    # Where the parallel_runner script lives — but we invoke it as a module,
    # so this path is informational only.
    ...
    def configure(
        ...
        """Inject runtime configuration. Call once at startup.

        Args:
            engine_config: EngineConfig with LLM credentials and zep flag
            run_state_dir: optional override for per-simulation storage dir
        """
        cls._engine_config = engine_config
        if run_state_dir:
            cls.RUN_STATE_DIR = os.path.abspath(run_state_dir)
        os.makedirs(cls.RUN_STATE_DIR, exist_ok=True)
    ...
    def start_simulation(
        ...
        # Optional Zep graph memory update (no-op if zep is not enabled)
        zep_on = bool(cls._engine_config and cls._engine_config.zep_enabled)
        if enable_graph_memory_update and zep_on:
            if not graph_id:
                raise ValueError("graph_id is required when enable_graph_memory_update=True")
            mgr = _load_zep_graph_memory_manager()
            if mgr is not None:
                try:
                    mgr.create_updater(simulation_id, graph_id)
                    cls._graph_memory_enabled[simulation_id] = True
                    logger.info(
                        f"Graph memory updater enabled: sim={simulation_id}, graph={graph_id}"
                    )
                except Exception as e:
            ...
            cls._graph_memory_enabled[simulation_id] = False
            if enable_graph_memory_update and not zep_on:
                logger.info("Skipping graph memory updater: Zep is disabled in EngineConfig")

        # MiroEdo only supports 'parallel' mode (twitter + reddit in same process).
        # The simulation runs as a Python module: `python -m app.engine.simulation.parallel_runner`.
        if platform not in ("parallel", "twitter", "reddit"):
            raise ValueError(f"Unsupported platform: {platform}")
        state.twitter_running = True
        state.reddit_running = True
        script_path = None  # not used; we invoke via -m
        ...
        try:
            ...
            cmd = [
                sys.executable,
                "-m",
                "app.engine.simulation.parallel_runner",
                "--config", config_path,
            ]
            if platform == "twitter":
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        try:
            ...
            # Stop graph memory updater (optional)
            if cls._graph_memory_enabled.get(simulation_id, False):
                if ZepGraphMemoryManager is not None:
                    try:
                        ZepGraphMemoryManager.stop_updater(simulation_id)
                        logger.info(f"Graph memory updater stopped: sim={simulation_id}")
                    except Exception as e:
    ...
    def _read_action_log(
        ...
        Returns:
            新的读取位置
        """
        # Check if graph memory update is enabled (optional)
        graph_memory_enabled = cls._graph_memory_enabled.get(state.simulation_id, False)
        graph_updater = None
        if graph_memory_enabled and ZepGraphMemoryManager is not None:
    ...
    def cleanup_all_simulations(cls):
        ...
        # First stop all graph memory updaters (no-op if zep is disabled)
        if ZepGraphMemoryManager is not None:
            try:
